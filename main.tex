...
\author{Stanley M. Porembski \footnote{Aerospace Engineer (Autonomous Vehicle Systems).}}
...
\begin{abstract}
The objective of this project is to develop an analysis tool for parametric studies of ramjet propulsion performance. FORTRAN was used as the primary coding language and the analysis tool was broken down into 6 modules following the states of the non-ideal Brayton cycle through a ramjet propulsion system along with overall ramjet performance parameters. The performance parameters were analyzed against different adjustable design parameters of the ramjet engine and plotted to analyze their trends. The tool was then used to design two ram/scramjet systems that would individually optimize the overall efficiency and total thrust. From the two designs it was found that both designs optimize the overall efficiency and total thrust simultaneously.
\end{abstract}
...
\subsection{\textit{T-s} Diagrams for Validation Cases} \label{sec:resultsparta} % Part A
