...
where the fuel-air mass ratio $f\approx0.06$ at $M_1=0.8$ and decreases to $f\approx0.03$ at $M_1=5$ which slightly raises the $V_{eq}^2$ term. The relation between the heat added in the combustor to the flight Mach number can be seen in Fig. \ref{fig:partcheat} where the amount of heat decreases due to the increasing total temperature $T_{t2}$ entering the combustor as the flow speed is decreased from $M_1$ to $M_2=0.15$. Since the combustor is not thermally choked, enough heat is able to be added so that the total temperature leaving the combustor reaches the material limits $T_{t3}=(T_{t3})_{max}=2400K$.
...
From Fig. \ref{fig:partcvelocity} and Fig. \ref{fig:partcheat}, the maximum thermal efficiency can be approximated to be $2\lessapprox\eta_{th}\lessapprox3$. Fig. \ref{fig:partcdimless} shows the individual efficiencies along with the fuel-air mass ratio.
