...
The ram/scramjet operate on the Brayton cycle. Oncoming airflow is compressed by the inlet/diffuser through ram compression. The kinetic energy of the air is converted to pressure energy as it passes through the diffuser \cite{rycey1996jetengine}. The pressure increase is dependent on the isentropic efficiency of the diffuser. For ramjets, the flow speed of the oncoming air is slowed to under Mach 0.4, and Scramjets are defined by the Mach entering the combustor being supersonic.

The airflow then has liquid fuel injected into it prior to entering the combustion chamber. The liquid fuel needs to be atomized, vaporized, and mixed in with the airflow before the combustion process occurs \cite{oates1997aerothermodynamics}.
...
The flow from the compressor then expands through the nozzle before exiting the nozzle and either expands or compresses to ambient pressure. The gas then cools to ambient temperatures.
...
\begin{table}[hbt!]
    ...
    \begin{tabular}{lcccc}
        \hline
            & \multicolumn{4}{c}{Ram/Scramjet Properties}\\
            Optimizing Parameter& $M_2$& $T_{t3}(K)$& $T_{total}(N)$& $\eta_o$\\\hline
...
\begin{figure}[hbt!] % Part I: propulsion design T-s diagram
    \centering
