...
The inlet/diffuser of the ramjet engine does not do work on or take work out of the oncoming air. It also does not transfer any heat into or out of the air, so the total temperature at state two is,
...
Then with the total temperature calculated, the given combustor inlet Mach number $M_2$ can be used to calculate the static temperature using the total-to-static relation,
...
The speed of sound and the flow speed entering the combustor can then be calculate from the computed state 2 static temperature using Eq. \ref{eqn:a2} and Eq. \ref{eqn:V2}, respectively.
...
To obtain the total pressure at state 2, the free stream Mach number $M_1$ and the given inlet/diffuser isentropic efficiency $\eta_d$ can be used in the following equation.
...
With the total pressure at state 2 calculated, the static pressure can be calculated from the total-to-static relation,
...
From the test Mach number, the state of the nozzle can be determined. If $M'<1$ then the nozzle is not choked and the exit Mach and static pressure can be calculated as,
...
However, if $M'\geq1$ then the nozzle flow is choked and the exit Mach and static pressure can instead be calcualted as,
...
where $c_p$ was calculated using Eq. \ref{eqn:latercp} for the exit temperature $T_e$. This entropy increase was then used to calculate the entropy increase from state 1 with,
...
Depending on the choked state of the converging only, the external nozzle flow efficiency can be calculated from the test Mach number that was calcualted using Eq. \ref{eqn:M'} as
...
Then using $\gamma=1.3$, the static temperature of the gas leaving the nozzle can be calculated using,
...
With the total and static temperature, the Mach number can be calculated for state 4 by rearranging the total-to-static relation for state 4 to,
...
Then with the Mach number, the total pressure of state 4 can be calculated from the total-to-static pressure relations,
...
The speed of sound and flow velocity can be calculated as,
...
where $c_p$ is calculated from Eq. \ref{eqn:latercp} and the entropy change from state 1 was calculated using,
...
The inlet mass flux can be calculated using $q_{23}$ calculated from Eq. \ref{eqn:q23} and the given $q_f$ using,
